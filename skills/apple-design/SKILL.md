...
description: >
  ...
  Flutter, React Native), desktop apps (macOS, Tauri, Electron), or web apps/dashboards
  (Next.js, React, Tailwind). Triggers when the user mentions: design review, UI audit, HIG
  compliance, improving app design, design feedback, accessibility audit, or any request to check
  or improve a design against professional standards. Also use proactively any time new UI is
  being built or refined for GotWurk (website, client portal, dashboards) — not only when
  explicitly asked to "review". Also use when the user uploads screenshots, mockups, wireframes,
  or design specs and wants feedback. Even if they just say "review my design" or "is this good
  UI", use this skill. Works for native and cross-platform frameworks including Flutter, Tauri,
  Electron, React Native, SwiftUI, AppKit/UIKit, and plain web (React/Next.js/Tailwind).
---

## GotWurk usage note

For GotWurk work specifically: apply these principles to *refine*, not *replace*, the existing
GotWurk visual identity. Keep GotWurk's established color palette, brand marks, and dark theme
exactly as-is — never propose swapping them for Apple's own palette or introducing iOS/macOS
chrome (traffic-light window buttons, SF Symbols, etc. — those are metaphors, not literal
components to import). What to actually pull from this skill for GotWurk: spacing/type rhythm and
hierarchy (`layout.md`, `typography.md`), motion and feedback quality (`motion.md`, `feedback.md`,
`loading.md`), accessibility rigor (`accessibility.md` — contrast, touch/click targets, focus
states), materials/elevation for cards and modals (`materials.md`, `dark-mode.md`), and drag/resize
interaction conventions (`drag-and-drop.md`) where relevant. Treat "web app / dashboard" as the
target platform and translate guidance using the desktop-app conventions in the table below
(sidebar nav, standard resizing/window behavior, keyboard shortcuts) unless the surface is a
marketing page, in which case treat it like a content site, not an app shell.
